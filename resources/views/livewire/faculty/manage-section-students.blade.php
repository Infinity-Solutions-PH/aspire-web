<div>
    ...
    <div class="flex items-center gap-2 mb-6">
        <a href="{{ route('faculty.sections') }}" class="flex items-center gap-1 text-[#9a4c4c] hover:text-primary transition-colors text-sm font-bold">
    ...
    <div class="bg-white dark:bg-[#2a1515] rounded-[32px] border border-[#f3e7e7] dark:border-[#3a1f1f] p-8 mb-8 shadow-sm relative overflow-hidden">
        ...
        <div class="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('faculty.sections.export.csv', $section->id) }}" class="flex items-center gap-2 h-11 px-5 rounded-xl bg-white dark:bg-[#361a1a] text-gray-700 dark:text-gray-300 font-bold text-sm shadow-sm hover:shadow-md transition-all border border-gray-200 dark:border-[#4d3232]">
                    <span class="material-symbols-outlined text-lg text-green-600">table_view</span>
                    Export CSV
                </a>
                <a href="{{ route('faculty.sections.export.pdf', $section->id) }}" target="_blank" class="flex items-center gap-2 h-11 px-5 rounded-xl bg-primary text-white font-bold text-sm shadow-sm hover:shadow-md hover:bg-primary/90 transition-all">
                ...
                <table class="w-full text-left">
                    <thead>
                        <tr class="bg-[#fdfafb] dark:bg-[#361a1a] border-b border-[#f3e7e7] dark:border-[#3a1f1f]">
                            ...
                            <th class="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-[#9a4c4c]">Guardian Name</th>
                    ...
                    <tbody class="divide-y divide-[#f3e7e7] dark:divide-[#3a1f1f]">
                        @forelse($students as $student)
                            <tr class="hover:bg-primary/[0.02] dark:hover:bg-white/[0.02] transition-colors group">
                                ...
                                <td class="px-6 py-4">
                                    <span class="text-xs font-bold text-gray-700 dark:text-gray-300">{{ $student->guardian_name ?? 'N/A' }}</span>
                                </td>
                                <td class="px-6 py-4">
