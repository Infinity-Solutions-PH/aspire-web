<section wire:key="step-5" class="glass-card rounded-xl shadow-sm border border-[#e7cfcf] dark:border-white/10 overflow-hidden">
    ...
    <div class="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
        <label wire:key="f-last-grade" class="flex flex-col gap-2">
            ...
            <p class="text-[9px] text-primary italic font-medium">Automatically set based on target grade level</p>
            @error('formData.last_grade_level') <span class="text-xs text-red-500 mt-1">{{ $message }}</span> @enderror
        </label>
        ...
        <label wire:key="f-last-gwa" class="flex flex-col gap-2">
            ...
            @error('formData.last_gwa') <span class="text-xs text-red-500 mt-1">{{ $message }}</span> @enderror
        </label>
        ...
        <div class="md:col-span-2 p-6 bg-primary/5 rounded-xl border border-primary/20 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                @foreach(['tech_voc_course1' => '1st', 'tech_voc_course2' => '2nd', 'tech_voc_course3' => '3rd'] as $var => $label)
                <div class="flex flex-col gap-1">
                    <select wire:model.live="formData.{{ $var }}" class="form-select text-xs rounded-lg border-[#e7cfcf] @error('formData.'.$var) border-red-500 @enderror">
                        <option value="" disabled>{{ $label }} Choice</option>
                        @foreach($allCourses as $course)
                            @php
                                $isAvailable = true;
                                if ($var === 'tech_voc_course2' && $course === $formData['tech_voc_course1']) $isAvailable = false;
                                if ($var === 'tech_voc_course3' && ($course === $formData['tech_voc_course1'] || $course === $formData['tech_voc_course2'])) $isAvailable = false;
                            @endphp
                            @if($isAvailable || $formData[$var] === $course)
                                <option value="{{ $course }}">{{ $course }}</option>
                            @endif
                        @endforeach
                    </select>
                    @error('formData.'.$var) <span class="text-[10px] text-red-500 font-medium">{{ $message }}</span> @enderror
                </div>
                @endforeach
            </div>
        </div>
