...
<head>
    ...
    <title>{{ $title ?? 'TNTS Faculty Portal' }}</title>
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed lg:sticky top-0 left-0 z-50 w-full lg:w-64 border-r border-[#e7cfcf] dark:border-[#3d2424] bg-background-light dark:bg-background-dark flex flex-col justify-between p-6 h-screen transition-transform duration-300 lg:translate-x-0">
            <div class="flex flex-col gap-8">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex flex-col">
                            ...
                            <p class="text-primary text-xs font-medium">Faculty Portal</p>
                        </div>
                    </div>
                ...
                <nav class="flex flex-col gap-2">
                    <a class="flex items-center gap-3 px-4 py-3 rounded-xl {{ request()->routeIs('faculty.dashboard') ? 'bg-primary/10 text-primary font-bold' : 'text-[#1b0d0d] dark:text-[#fcf8f8] hover:bg-gray-100 dark:hover:bg-white/5' }} transition-colors" href="{{ route('faculty.dashboard') }}">
                        <span class="material-symbols-outlined">dashboard</span>
                        <span class="text-sm">Dashboard</span>
                    </a>
                    <a class="flex items-center gap-3 px-4 py-3 rounded-xl {{ request()->routeIs('faculty.sections') || request()->routeIs('faculty.sections.students') ? 'bg-primary/10 text-primary font-bold' : 'text-[#1b0d0d] dark:text-[#fcf8f8] hover:bg-gray-100 dark:hover:bg-white/5' }} transition-colors" href="{{ route('faculty.sections') }}">
                        <span class="material-symbols-outlined">groups</span>
                        <span class="text-sm">Handled Sections</span>
                    </a>
                    <a class="flex items-center gap-3 px-4 py-3 rounded-xl {{ request()->routeIs('faculty.profile') ? 'bg-primary/10 text-primary font-bold' : 'text-[#1b0d0d] dark:text-[#fcf8f8] hover:bg-gray-100 dark:hover:bg-white/5' }} transition-colors" href="{{ route('faculty.profile') }}">
            ...
            <div class="flex flex-col gap-4">
                <div class="flex items-center gap-3 p-3 rounded-xl bg-[#f3e7e7] dark:bg-[#3d2424]">
                    ...
                    <div class="overflow-hidden">
                        ...
                        <p class="text-[10px] text-primary">Faculty Account</p>
                    </div>
                </div>
            ...
            <header class="flex items-center justify-between px-4 lg:px-8 py-4 bg-background-light dark:bg-background-dark border-b border-[#e7cfcf] dark:border-[#3d2424]">
                <div class="flex items-center gap-3 lg:gap-4 flex-1 max-w-xl">
                    ...
                    <div class="relative w-full hidden sm:block">
                        ...
                        <input class="w-full bg-[#f3e7e7] dark:bg-[#3d2424] border-none rounded-xl pl-10 pr-4 py-2 text-sm focus:ring-2 focus:ring-primary/50 transition-all placeholder:text-gray-500" placeholder="Search sections or students..." type="text"/>
                    </div>
                </div>
            </header>
